...
if __name__ == '__main__':
    ...
    conf = SparkConf().setAppName('Angel Murcia Diaz -> Practica 4 - Random Forest - Dataset Balanceado OverSamplig - 100 arboles')  
    ...
    #Balanceo de los datos utilizando OverSampling
    NoDataset = dataset.filter('label=0')
    SiDataset = dataset.filter('label=1')
    dataset = NoDataset.unionAll(SiDataset)
    dataset = dataset.unionAll(SiDataset)
